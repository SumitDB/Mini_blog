...
def Dashboard(request):
    if request.user.is_authenticated:
        ...
        gps = user.groups.all()
        return render(request, 'reg/dashboard.html', {'posts':posts, 'full_name':full_name,'groups':gps})
    else:
...
def add_post(request):
    if request.user.is_authenticated:
        if request.method == 'POST':
            ...
            form = PostForm()
        return render(request, 'reg/addpost.html', {'form':form})
    else:
        return HttpResponseRedirect('/login/')
...
def update_post(request, id):
    if request.user.is_authenticated:
        ...
        return render(request, 'reg/updatepost.html', {'form':form})
    else:
        return HttpResponseRedirect('/login/')
